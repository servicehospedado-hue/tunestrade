...
async def run_monitoring_demo(ssid: Optional[str] = None):
    ...
    if not ssid:
        # Obter SSID de variável de ambiente
        import os
        ssid = os.getenv("POCKETOPTION_DEMO_SSID")
        if not ssid:
            logger.error("Erro: SSID não fornecido. Use: python connection_monitor.py 'seu_ssid'")
            logger.info("Ou defina POCKETOPTION_DEMO_SSID no arquivo .env")
            return
