    ...
    async def _establish_connection(self) -> bool:
        ...
        for attempt in range(len(self.available_urls)):
            ...
            try:
                ...
                # SSL context - usar default seguro (não desabilitar verificação)
                import ssl

                ssl_context = ssl.create_default_context()
                # Permitir apenas TLS 1.2+
...
async def demo_keep_alive():
    ...
    # Obter SSID de variável de ambiente (nunca hardcode em produção!)
    import os
    ssid = os.getenv("POCKETOPTION_DEMO_SSID")
    if not ssid:
        logger.error("Erro: POCKETOPTION_DEMO_SSID não definido no .env")
        logger.info("Crie um arquivo .env com: POCKETOPTION_DEMO_SSID=seu_ssid_aqui")
        return
